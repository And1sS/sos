...
static bool hash_table_grow(hash_table* table) {
    ...
    for (u64 i = 0; i < table->buckets_num; i++) {
        ...
        while (entry_node != NULL) {
            hash_table_entry* entry = (hash_table_entry*) entry_node->value;
...
static linked_list_node* hash_table_bucket_lookup(linked_list* bucket,
                                                  u64 key) {
...
void* hash_table_get(hash_table* table, u64 key) {
    linked_list* bucket = &table->buckets[key % table->buckets_num];
    linked_list_node* entry_node = hash_table_bucket_lookup(bucket, key);
    return entry_node ? ((hash_table_entry*) entry_node->value)->value : NULL;
}

bool hash_table_contains(hash_table* table, u64 key) {
    linked_list* bucket = &table->buckets[key % table->buckets_num];
    return hash_table_bucket_lookup(bucket, key) != NULL;
}
...
bool hash_table_put(hash_table* table, u64 key, void* value, void** old_value) {
    ...
    hash_table_entry* new_entry =
        (hash_table_entry*) kmalloc(sizeof(hash_table_entry));
    new_entry->key = key;
