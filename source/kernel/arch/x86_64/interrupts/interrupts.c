...
struct cpu_context* syscall_trampoline(struct cpu_context* context) {
    ...
    return handle_syscall(arch_context->rdi, arch_context->rsi,
                          arch_context->rdx, arch_context->rcx,
                          arch_context->r8, arch_context->r9, arch_context->rax,
                          context);
}
...
void interrupts_init() {
    ...
    mount_irq_handler(14, handle_page_fault);
